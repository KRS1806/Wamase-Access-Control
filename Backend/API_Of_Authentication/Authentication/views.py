...
class MarkWithFACEIDAPIView(APIView):
    parser_classes = [MultiPartParser, FormParser, JSONParser]
    ...
    def post(self, request, *args, **kwargs):
        ...
        # Convertimos la imagen binaria de entrada en una imagen RGB
        img_bytes_new = face_photo.read()
        img_np = np.frombuffer(img_bytes_new, np.uint8)
        img_new = cv2.imdecode(img_np, cv2.IMREAD_COLOR)

        # Extraemos la codificación de la imagen subida
        unknown_face_encodings = face_recognition.face_encodings(img_new)

        if len(unknown_face_encodings) == 0:
            return JsonResponse({'message': 'No se detecto ningun rostro'}, status=400)

        unknown_encoding = unknown_face_encodings[0]
        threshold = 0.5  # Ajusta este valor según tus necesidades

        # Iteramos sobre cada usuario y verificamos la coincidencia de la cara
        users = User_Worker.objects.all()
        for user_search in users:
            # Convertimos la imagen binaria de la base de datos en una imagen RGB
            photo_save = user_search.photo
            img_binary = np.frombuffer(photo_save, np.uint8)
            img_save = cv2.imdecode(img_binary, cv2.IMREAD_COLOR)

            # Extraemos la codificación de la imagen guardada
            known_face_encodings = face_recognition.face_encodings(img_save)
            ...
            # Si la distancia de coincidencia es menor o igual al umbral, registramos la marca
            if face_distance <= threshold:
                return self.handle_mark(user_search, in_out)

        return Response({'message': 'Rostro no identificado'}, status=404)

    def handle_mark(self, user_search, in_out):
        today = date.today()
        last_mark = CreateMark.objects.filter(user=user_search, date=today).order_by('-id').first()
        all_marks_today = CreateMark.objects.filter(user=user_search, date=today)

        if in_out == "Entrada":
            if last_mark and last_mark.in_out == "Entrada":
                data = {
                    'message': f"Ya tienes un ingreso registrado para hoy {user_search.name}",
                }
                return Response(data, status=404) 

            hours_entry = datetime.now().time().strftime("%H:%M")
            late = float(hours_entry.replace(":", ".")) > user_search.hourIn

            CreateMark.objects.create(user=user_search, in_out=in_out, hours=hours_entry.replace(".", ":"), hours_extras=0, total_hours=0, in_late=late)
            data = {
                'message': f"Ingreso registrado {user_search.name}",
            ...
            if last_mark and last_mark.in_out == "Salida":
                data = {
                    'message': f"Ya tienes una salida registrada para hoy {user_search.name}",
                }
                return Response(data, status=404)

            marks_in = CreateMark.objects.filter(user=user_search, in_out="Entrada", date=today).last()
            if not marks_in:
                return Response(f"No tienes una marca de entrada registrada para hoy {user_search.name}", status=404)
            ...
            exit_hours = final_hours
            total_hours = calcular_diferencia_decimal(entry_hours, exit_hours)
            total_hours_marks = sum(mark.total_hours for mark in all_marks_today)

            if total_hours_marks >= 7.60:
                extras = total_hours
                late = float(hours.replace(":", ".")) > user_search.hourOut
                CreateMark.objects.create(user=user_search, in_out=in_out, hours=hours, hours_extras=extras, total_hours=0, out_late=late)
            else:
                if total_hours == 0.01:
                    total_hours = 0.01
                ...
                if total_hours > 8:
                    ...
                    extras = 0
                late = float(hours.replace(":", ".")) > user_search.hourOut
                CreateMark.objects.create(user=user_search, in_out=in_out, hours=hours, hours_extras=extras, total_hours=total_hours, out_late=late)
            data = {
                'message': f"Salida registrada {user_search.name}",
            }
            return Response(data, status=200)
...
class RegisterAPIView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            # User_Worker.objects.create(name=name, last_name=last_name, departament=departament, user=user, salary=salary, extras=extras, hourIn=float(hourIn), hourOut=float(hourOut), photo=photo_binary, fingerPrint_template=fingerprint_data, face=face_photo_read)
            User_Worker.objects.create(name=name, last_name=last_name, departament=departament, user=user, salary=salary, extras=extras, hourIn=float(hourIn), hourOut=float(hourOut), photo=photo_binary, face=photo_binary)
            data = {
